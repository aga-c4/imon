...
from models.sourcemysql import Sourcemysql
from models.sysbf import SysBf

class Robot_mysql1get:
    ...
    def __init__(self, *, settings:None, config:dict={}):
        ...
        if 'sources' in config:
            ...
            self.metrics_pkg_qty = 1 # int(config['sources'][self.source_alias].get('metrics_pkg_qty', self.metrics_pkg_qty))
            self.tz_str_source = config['sources'][self.source_alias].get('timezone', self.tz_str_source)
            if 'db' in config['sources'][self.source_alias]:
                self.db_source = Mysqldb(config['sources'][self.source_alias]['db'], 'mysql1')
        ...
        for granularity, granularity_settings in self.granularity_list.items():
            ...
            for metric_group in metric_groups:
                ...
                for metric_api_alias, metric in metrics.items():
                    metrics_list.append(metric['metric_api_alias']) 
                    ...
                    if metric['accuracy']<1 or counter >= self.metrics_pkg_qty or metric_api_alias == last_metric: # Пачку собрали, обрабатываем
                        ...
                        filename = self.tmp_path + '/mysql1_' + granularity + '_' + metric_api_alias + '_' + start_dt_str + '_' + end_date_f + '.json'
                        ...
                        if not self.fr_api and not os.path.exists(filename): # Принудительный забор или еще не забирали и нет файла, то и кладем в файл
                            logging.info(f'Mysql1 start get to: {filename}')
                            api_timer = SysTimer()
                            response = Sourcemysql.get_data(db=self.db_source, granularity=granularity,
                                            dt_from_str=start_dt_str, dt_to_str=end_date, 
                                            tz_str=self.tz_str_source, fileto=filename, metrics_list=metrics_list)
                            ...
                            logging.info(f'Mysql1 complite get to: {filename}')
                            metrics_list = []
