    ...
    def run(self, *, output:bool=False) -> dict:
        ...
        for granularity, granularity_settings in self.granularity_list.items():
            ...
            for metric_group in metric_groups:
                ...
                metrics_dict = {}
                metrics = Metric.get_list(db=db, source_alias=source_alias, group_id=metric_group['id'], metric_type="src", order='accuracy,metric_api_filters')
                ...
                for mt in metrics:
                    ...
                    if self.settings['metric_id']>0 and self.settings['metric_id']!=mt['id']:
                        continue
                    # у одинаковых алиасов метрик могут быть разные фильтры, поэтому в ключ мы добавим фильтр
                    metrics_dict[mt['metric_api_alias']+mt['metric_api_filters']] = mt
                ...
                metrics_fstr = ''
                min_max_dt = None
                last_api_filters = None
                lastmetric = None
                for upd_metric, metric in metrics.items():
                    if last_api_filters is None:
                        last_api_filters = metric['metric_api_filters']
                    if counter>0 and not lastmetric is None and (metric['accuracy']<1 \
                                      or metric['metric_api_filters']!=last_api_filters \
                                        or counter >= self.metrics_pkg_qty):
                        # Пачку собрали, обрабатываем
                        res = self.add_values(db=db,
                                        ...
                                        metrics_fstr=metrics_fstr,
                                        source_alias=source_alias,
                                        source_id=source_id
                                        )
                        insert_counter_all += res['insert_counter_all']
                        upd_counter_all += res['upd_counter_all']  
                        logging.info(f"Finish {granularity}::{metric_group['metric_group_alias']}::{metrics_str}")                            
                ...
                # Запишем оставшуюся часть пачки
                if not lastmetric is None:
                    res = self.add_values(db=db,
                                        min_max_dt=min_max_dt, 
                                        datetime_now=datetime_now, 
                                        datetime_to=datetime_to,
