...
class Robot_mgen:
    ...
    def run(self, *, output:bool=False) -> dict:
        ...
        self.comment(f"mgen::{self.alias}::{self.settings['granularity']}::{settings_project_id}")        
        ...
        if os.path.exists(proc_file0) or os.path.exists(proc_file):
            ...
            if last_proc_dt_str=='' or SysBf.tzdt_fr_str(last_proc_dt_str, self.tz_str_system) > (datetime_now - timedelta(seconds=self.proc_ttl)):
                ...
                self.comment(f"Project {settings_project_id} Gran:[{self.settings['granularity']}] Error: Already running or process file error!")  
                        ...
                        for cur_tag in metric_tags:
                            ...
                            for mt in cur_res_metrics:
                                ...
                                if dt_start_gen >= dt_fin_gen:
                                    logging.info(f"Calculate Project: {project_id}, Gr: {group_id}, mt_id: {mt['id']}, TgId: {tag_id}: Already calculated (dt_start_gen{str(dt_start_gen)} >= dt_fin_gen{str(dt_fin_gen)})!")
                                    continue             
                                logging.info(f"Calculate Project: {project_id}, Gr: {group_id}, mt_id: {mt['id']}, TgId: {tag_id}, dt_start_gen: {str(dt_start_gen)}, dt_start_ins: {str(dt_start_ins)}, dt_fin_gen: {str(dt_fin_gen)}")    
